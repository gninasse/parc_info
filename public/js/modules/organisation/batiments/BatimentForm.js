/**
 * BatimentForm.js
 * Handles Modal, Form Validation, and Submission for Batiments.
 */
export class BatimentForm {
    ...
    openForAdd() {
        this.resetForm();
        $('#createBatimentModalLabel').text('Nouveau Bâtiment');
        $('#batiment_id').val('');
        this.$modal.modal('show');
    }

    openForEdit(data) {
        $('#createBatimentModalLabel').text('Modifier le Bâtiment');
        $('#batiment_id').val(data.id);
        $('#site_id').val(data.site_id);
        $('#code').val(data.code);
        $('#libelle').val(data.libelle);
        $('#nombre_etages').val(data.nombre_etages);
        $('#description').val(data.description);
        this.$modal.modal('show');
    }

    initSubmission() {
        $('#batimentForm').on('submit', (e) => {
            e.preventDefault();
            ...
            const batimentId = $('#batiment_id').val();
            const url = batimentId
                ? route('organisation.batiments.update', batimentId)
                : route('organisation.batiments.store');
            const method = batimentId ? 'PUT' : 'POST';
            ...
            $.ajax({
                ...
                beforeSend: () => {
                    $('#btn-save-batiment').prop('disabled', true).html('<i class="fas fa-spinner fa-spin"></i> Enregistrement...');
                },
                success: (response) => {
                    if (response.success) {
                ...
                complete: () => {
                    $('#btn-save-batiment').prop('disabled', false).html('<i class="fas fa-save"></i> Enregistrer');
                }
            });
        });
    }
