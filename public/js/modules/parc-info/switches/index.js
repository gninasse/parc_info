/**
 * index.js — Switches (ParcInfo)
 */

// ── Formatters Bootstrap Table ────────────────────────────────────────────────

window.switchesQueryParams = function (params) {
...
window.codeFormatter = (val, row) =>
    `<a href="${route('parc-info.switches.show', row.id)}" class="fw-bold text-primary small text-decoration-none">${val}</a>`;
...
window.poeFormatter = (val) => {
    return val
        ? `<span class="badge bg-info-subtle text-info border border-info-subtle px-2 py-1">PoE</span>`
        : `<span class="text-muted small">Non</span>`;
};

window.actionsFormatter = (id) =>
    `<div class="d-flex gap-1">
        <a href="${route('parc-info.switches.show', id)}" class="btn btn-sm btn-outline-secondary border-0" title="Voir / Modifier"><i class="bi bi-eye"></i></a>
...
window.actionsEvents = {
    'click [data-action="delete"]': (e, val, row) => deleteSwitch(row.id),
};

// ── KPI ───────────────────────────────────────────────────────────────────────

function loadKpis() {
    $.get(route('parc-info.switches.data'), { limit: 9999, offset: 0 }, (res) => {
        const rows = res.rows ?? [];
...
const Wizard = (() => {
    ...
    function reset() {
        $form()[0].reset();
        $('#sw_id').val('');
        $('#wizard-title').text('Ajouter un switch');
    ...
    function init() {
        ...
        // QuickAdd Marque
        $('#btn-add-marque').on('click', () => quickAdd('Nouvelle marque', 'Ex: Cisco, HP, TP-Link...', 'parc-info.switches.store-marque', 'marque_id'));

        function quickAdd(title, placeholder, routeName, selectId) {
            const bsModal = bootstrap.Modal.getInstance(document.getElementById('switchModal'));
        ...
        $form().on('submit', function(e){
            ...
            $.ajax({
                url: route('parc-info.switches.store'),
                method: 'POST',
                data: $.param(formData),
                success: (res) => {
                    $modal().modal('hide');
                    $('#switches-table').bootstrapTable('refresh');
                    loadKpis();
                    Swal.fire({ icon: 'success', title: 'Enregistré', timer: 2000, showConfirmButton: false });
                },
                error: (xhr) => {
                    $btn.prop('disabled', false).find('#btn-submit-label').text('Enregistrer le switch');
                    $form().find('.is-invalid').removeClass('is-invalid');
                    $form().find('.invalid-feedback').remove();
...
function deleteSwitch(id) {
    Swal.fire({
        ...
        if (res.isConfirmed) {
            $.ajax({
                url: route('parc-info.switches.destroy', id),
                method: 'DELETE',
...
$(function () {
    ...
    $('#btn-apply-filters').on('click', () => $('#switches-table').bootstrapTable('refresh'));
    $('#btn-reset-filters').on('click', () => { $('#filter-site, #filter-direction, #filter-statut').val(''); $('#switches-table').bootstrapTable('refresh'); });

    $('#switches-table').on('check.bs.table uncheck.bs.table load-success.bs.table', function () {
        const sel = $(this).bootstrapTable('getSelections');
        $('#btn-edit, #btn-delete').prop('disabled', sel.length === 0);
    });

    $('#btn-edit').on('click', () => {
        const sel = $('#switches-table').bootstrapTable('getSelections');
        if(sel.length) window.location.href = route('parc-info.switches.show', sel[0].id);
    });

    $('#btn-delete').on('click', () => {
        const sel = $('#switches-table').bootstrapTable('getSelections');
        if(sel.length) deleteSwitch(sel[0].id);
    });
});
