/**
 * Gestion des Types de Consommable - Module Parc Info
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function editItem(id) {
        $.ajax({
            url: route('parc-info.referentiels.types-consommables.show', id),
            method: 'GET',
            success: function(res) {
                if (res.success) {
                    const d = res.data;
                    $('#item-id').val(d.id);
                $form.find('[name="code"]').val(d.code);
                $form.find('[name="nom"]').val(d.nom);
                $form.find('[name="categorie"]').val(d.categorie);
                $form.find('[name="sous_categorie"]').val(d.sous_categorie);
                $form.find('[name="unite_stock"]').val(d.unite_stock);
                $form.find('[name="seul_reapprovisionnement"]').val(d.seul_reapprovisionnement);
                $form.find('[name="duree_conservation_jours"]').val(d.duree_conservation_jours);
    ...
    $form.on('submit', function(e) {
        ...
        const url = id 
            ? route('parc-info.referentiels.types-consommables.update', id) 
            : route('parc-info.referentiels.types-consommables.store');
        ...
        $btnDelete.on('click', function() {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: route('parc-info.referentiels.types-consommables.destroy', row.id),
                        method: 'DELETE',
                        data: {
