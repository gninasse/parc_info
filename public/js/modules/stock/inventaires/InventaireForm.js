export class InventaireForm {
    ...
    _initSubmission() {
        this.$form.on('submit', (e) => {
            e.preventDefault();
            const url = route('stock.inventaires.store');
            const $btn = $('#btn-save-inventaire-submit');

            $btn.prop('disabled', true).html('<i class="fas fa-spinner fa-spin"></i> Initialisation...');
            ...
            $.ajax({
                ...
                success: (res) => {
                    if (res.success) {
                        ...
                        Swal.fire({
                            ...
                            confirmButtonText: 'Saisir les comptages',
                            cancelButtonText: 'Fermer'
                        }).then((result) => {
                            if (result.isConfirmed) {
                                window.location.href = route('stock.inventaires.saisie', res.data.id);
                            }
                        });
                    }
                },
                ...
                complete: () => {
                    $btn.prop('disabled', false).html('<i class="fas fa-play me-1"></i> Démarrer la campagne');
                }
            });
        });
    }
