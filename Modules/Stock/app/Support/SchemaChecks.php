 ...
 * actives sur PostgreSQL ET SQLite (exigence SFD §9.3 « double filet » :
 * les gardes applicatives sont doublées par la base).
 *
 * PostgreSQL accepte ALTER TABLE … ADD CONSTRAINT … CHECK. SQLite ne sait
 * pas ajouter un CHECK a posteriori : la table (encore vide, on est dans
 * sa propre migration) est recréée avec les CHECK intégrés au CREATE TABLE,
 * puis ses index sont rejoués à l'identique.
 *
 * Les expressions doivent rester portables : pas de fonction propre à un
 * SGBD (num_nonnulls…) — utiliser CASE WHEN.
 */
    ...
    public static function ajouter(string $table, array $checks): void
    {
        ...
        match (DB::getDriverName()) {
            ...
            default => throw new \RuntimeException(
                'SGBD non supporté pour les CHECK du module Stock : '.DB::getDriverName()
            ),
        };
    }
