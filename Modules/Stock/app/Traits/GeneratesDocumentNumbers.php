 ...
 *
 * La séquence est portée par une table dédiée et verrouillée (lockForUpdate)
 * le temps de la transaction : deux créations concurrentes ne peuvent pas
 * obtenir le même numéro. Le comptage des lignes existantes, non sûr, n'est
 * jamais utilisé.
 *
 * Format : {PREFIXE}-{ANNEE}-{SEQUENCE sur 4 chiffres} (RG-F5-07, RG-F6-10)
 */
trait GeneratesDocumentNumbers
{
    public function genererNumero(string $type, string $prefix, ?int $annee = null): string
    {
        ...
        return DB::transaction(function () use ($type, $prefix, $annee) {
            $sequence = DB::table('stock_sequences')
                ->where('type', $type)
                ->where('annee', $annee)
                ->lockForUpdate()
                ->first();
