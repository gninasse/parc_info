...
<div class="card border-0 shadow-sm mb-3">
    <div class="card-body">
        <div class="row g-3 align-items-end">
            ...
            <div class="col-md-3">
                <label class="form-label small text-uppercase text-muted" for="filter-magasin">Magasin (source ou cible)</label>
            ...
            <div class="col-md-3">
                {{-- Toggle ?cible=mon-magasin : visible si l'utilisateur est responsable d'un magasin --}}
                @if($monMagasin)
                    <div class="form-check form-switch">
                        <input class="form-check-input" type="checkbox" id="filter-mon-magasin">
                        <label class="form-check-label small" for="filter-mon-magasin">
                            Vers mon magasin ({{ $monMagasin->libelle }})
                        </label>
                    </div>
                @endif
            </div>
...
<div class="card border-0 shadow-sm">
    <div class="card-header bg-white border-0 py-3">
        <h6 class="mb-0 fw-bold"><i class="bi bi-arrow-left-right me-2 text-primary"></i>Bons de transfert</h6>
    </div>
    <div class="card-body p-0">
        <div id="toolbar">
            @can('stock.transferts.store')
            <a href="{{ route('stock.transferts.create') }}" id="btn-add" class="btn btn-primary btn-sm" data-bs-toggle="tooltip" title="Ajouter">
                <i class="fas fa-plus"></i>
            </a>
            @endcan
            @can('stock.transferts.index')
            <button id="btn-show" class="btn btn-secondary btn-sm" disabled data-bs-toggle="tooltip" title="Voir">
                <i class="fas fa-eye"></i>
            </button>
            @endcan
            @can('stock.transferts.update')
        ...
        <table id="transferts-table"
            ...
            <thead class="table-light">
                <tr>
                    ...
                    <th data-field="magasin_source">Source</th>
                    <th data-field="magasin_cible">Cible</th>
                    <th data-field="transporte_par">Transporté par</th>
