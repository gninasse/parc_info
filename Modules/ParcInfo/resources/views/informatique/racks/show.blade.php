...
$(document).ready(function() {
    ...
    $('#form-update').on('submit', function(e) {
        ...
        $.ajax({
            url: `{{ route('parc-info.racks.update', $equipement->id) }}`,
            method: 'PUT',
            data: $(this).serialize(),
            success: (res) => {
                if (res.success) {
    ...
    $('.dropdown-item[data-statut]').on('click', function (e) {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: `{{ route('parc-info.racks.update-statut', $equipement->id) }}`,
                    method: 'PATCH',
    ...
    $('#btn-desaffecter').on('click', function () {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: `{{ route('parc-info.racks.desaffecter', $equipement->id) }}`,
                    method: 'POST',
