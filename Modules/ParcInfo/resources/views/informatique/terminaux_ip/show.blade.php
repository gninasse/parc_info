...
@section('breadcrumb')
    ...
    <li class="breadcrumb-item"><a href="{{ route('parc-info.terminaux-ip.index') }}">Terminaux IP</a></li>
        ...
        <div class="row align-items-center g-3">
            ...
            <div class="col">
                ...
                <div class="d-flex gap-4 flex-wrap text-muted small">
                    ...
                    @if($reseau?->typeReseau)
                    <span><i class="bi bi-tag me-1"></i>{{ $reseau->typeReseau->libelle }}</span>
                    @endif
            ...
            <div class="col-auto d-flex gap-2">
                ...
                <div class="dropdown">
                    <button class="btn btn-outline-warning btn-sm dropdown-toggle" type="button" data-bs-toggle="dropdown">
                        <i class="bi bi-activity me-1"></i> État
                    </button>
                    <ul class="dropdown-menu shadow border-0">
                        @foreach(['bon'=>'Bon','passable'=>'Passable','mauvais'=>'Mauvais','avarie'=>'Avarié'] as $k=>$v)
                        <li><a class="dropdown-item py-2" href="#" data-action="update-etat" data-value="{{ $k }}">{{ $v }}</a></li>
                        @endforeach
                    </ul>
                </div>
                @if($aff)
                    ...
                    <div class="card border-0 shadow-sm mb-3">
                        ...
                        <div class="card-body">
                            <div class="row g-3">
                                ...
                                {{-- Technical specific --}}
                                <div class="col-md-6">
                                    <label class="form-label small fw-bold text-muted">Type de Terminal / Réseau</label>
                                    <select class="form-select" name="type_reseau_id" disabled id="f_type_reseau_id">
                                        <option value="">Sélectionner...</option>
                                        @foreach($typesReseaux as $t)
                                        <option value="{{ $t->id }}" {{ $t->id == $reseau?->type_reseau_id ? 'selected' : '' }}>{{ $t->libelle }}</option>
                                        @endforeach
                                    </select>
                                </div>
                                <div class="col-md-6">
                                    <label class="form-label small fw-bold text-muted">Adresse IP</label>
                                    <input type="text" class="form-control" name="adresse_ip" value="{{ $reseau->adresse_ip ?? '' }}" disabled id="f_adresse_ip" placeholder="Ex: 10.0.10.45">
                                </div>
                                <div class="col-md-6">
                                    <label class="form-label small fw-bold text-muted">Masque de sous-réseau</label>
                                ...
                                <div class="col-md-6">
                                    ...
                                    <input type="text" class="form-control" name="passerelle" value="{{ $reseau->passerelle ?? '' }}" disabled id="f_passerelle" placeholder="Ex: 10.0.10.1">
                                </div>
                                <div class="col-md-6">
                                    <label class="form-label small fw-bold text-muted">Communauté SNMP</label>
                                    <input type="text" class="form-control" name="communaute_snmp" value="{{ $reseau->communaute_snmp ?? '' }}" disabled id="f_communaute_snmp" placeholder="Ex: public">
                                </div>
                                <div class="col-md-6">
                                    <label class="form-label small fw-bold text-muted">ID VLAN Management</label>
                                    <input type="number" class="form-control" name="vlan_management" value="{{ $reseau->vlan_management ?? '' }}" disabled id="f_vlan_management" min="0">
                                </div>
                                <div class="col-md-6">
                                    <label class="form-label small fw-bold text-muted">Version du Firmware</label>
                                    <input type="text" class="form-control" name="version_firmware" value="{{ $reseau->version_firmware ?? '' }}" disabled id="f_version_firmware" placeholder="Ex: v1.4.2-build89">
                                </div>
                                <div class="col-md-6 d-flex align-items-center pt-4">
                                    <div class="form-check form-switch">
                                        <input class="form-check-input" type="checkbox" name="est_manageable" value="1" {{ ($reseau->est_manageable ?? false) ? 'checked' : '' }} disabled id="f_est_manageable">
                                        <label class="form-check-label small fw-bold text-muted" for="f_est_manageable">Terminal Gérable (SNMP/Web)</label>
                                    </div>
                                </div>
                            </div>
                        </div>
        ...
        <div class="card border-0 shadow-sm" style="border-radius:12px">
            <div class="card-body p-4">
                ...
                <div class="text-center py-5">
                    ...
                    <p class="text-muted fw-semibold">Ce terminal IP n'est affecté à aucun local ou bureau</p>
        ...
        <div class="modal-content border-0 shadow-lg" style="border-radius:14px">
            ...
            <form id="affectationForm">
                ...
                <div class="modal-body px-4 py-3">
                    ...
                    <p class="text-muted small mb-4">Associez ce terminal IP à un local ou bureau spécifique.</p>
...
$(function(){
    ...
    $('#btn-save-fiche').on('click', function(){
        ...
        const estManageableIndex = data.findIndex(item => item.name === 'est_manageable');
        if (estManageableIndex > -1) {
            data[estManageableIndex].value = $('#f_est_manageable').is(':checked') ? '1' : '0';
        } else {
            data.push({ name: 'est_manageable', value: $('#f_est_manageable').is(':checked') ? '1' : '0' });
        }

        $.ajax({
            url: route('parc-info.terminaux-ip.update', '{{ $equipement->id }}'),
            method: 'PUT',
            data: $.param(data),
            success: (res) => {
                if(res.success){
    ...
    $('[data-action="update-statut"]').on('click', function(e){
        ...
        Swal.fire({
            ...
            input: 'textarea',
            inputPlaceholder: 'Ex: Remplacement matériel, Réparation en cours...',
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: route('parc-info.terminaux-ip.update-statut', '{{ $equipement->id }}'),
                    method: 'PATCH',
    ...
    // Mise à jour État (Header)
    $('[data-action="update-etat"]').on('click', function(e){
        e.preventDefault();
        const etat = $(this).data('value');
        Swal.fire({
            title: 'Changer l\'état général',
            text: 'Veuillez saisir le motif du changement d\'état :',
            input: 'textarea',
            inputPlaceholder: 'Ex: Suite à un choc physique, Matériel dégradé...',
            showCancelButton: true,
            confirmButtonText: 'Mettre à jour',
            cancelButtonText: 'Annuler',
            preConfirm: (motif) => {
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: route('parc-info.terminaux-ip.update-etat', '{{ $equipement->id }}'),
                    method: 'PATCH',
                    data: { _token: '{{ csrf_token() }}', etat: etat, motif: result.value },
                    success: () => location.reload(),
                    error: () => Swal.fire('Erreur', 'Impossible de mettre à jour l\'état.', 'error')
                });
            }
        });
    });
    ...
    $('#btn-desaffecter').on('click', function(){
        Swal.fire({
            title: 'Désaffecter le Terminal IP ?',
            text: 'Le terminal sera remis en stock. Indiquez le motif :',
            input: 'textarea',
            showCancelButton: true,
            ...
            if (result.isConfirmed) {
                $.post(route('parc-info.terminaux-ip.desaffecter', '{{ $equipement->id }}'), {
    ...
    $('#affectationForm').on('submit', function(e){
        ...
        $.post(route('parc-info.terminaux-ip.store-affectation'), $(this).serialize(), (res) => {
            if (res.success) {
