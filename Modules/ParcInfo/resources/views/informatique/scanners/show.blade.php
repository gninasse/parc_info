        ...
        <div class="row align-items-center g-3">
            ...
            <div class="col">
                ...
                <div class="d-flex gap-4 flex-wrap text-muted small">
                    ...
                    @if($s?->resolution_dpi_max)
                    <span><i class="bi bi-aspect-ratio me-1"></i>{{ $s->resolution_dpi_max }} DPI</span>
                    @endif
                    @if($s?->format_max)
                    <span><i class="bi bi-file-earmark me-1"></i>{{ $s->format_max }}</span>
                    @endif
                </div>
            </div>
    ...
    <form id="ficheForm">
                ...
                <div class="row g-3">
                    ...
                    <div class="col-md-4">
                        ...
                        <div class="input-group">
                            ...
                            <button type="button" class="btn btn-outline-secondary d-none" id="btn-add-marque-show" title="Nouvelle marque"><i class="bi bi-plus"></i></button>
                        </div>
                    </div>
                    <div class="col-md-4">
        ...
        <div class="card border-0 shadow-sm mb-3" style="border-radius:12px">
            <div class="card-body p-4">
                ...
                <div class="row g-3">
                    <div class="col-md-4">
                        <label class="field-label">Résolution DPI max</label>
                        <input type="number" class="form-control field-input" name="resolution_dpi_max"
                               value="{{ $s?->resolution_dpi_max }}" placeholder="Ex: 600" disabled>
                    </div>
                    <div class="col-md-4">
                        <label class="field-label">Format maximum</label>
                        <select class="form-select field-input" name="format_max" disabled>
                            <option value="">Sélectionner...</option>
                            @foreach(['A4','A3','A2','Lettre','Légal'] as $f)
                            <option value="{{ $f }}" {{ $s?->format_max === $f ? 'selected':'' }}>{{ $f }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="col-md-4">
                        <label class="field-label">Type de capteur</label>
                        <select class="form-select field-input" name="type_capteur" disabled>
                            <option value="">Sélectionner...</option>
                            @foreach(['CCD','CIS','CMOS'] as $t)
                            <option value="{{ $t }}" {{ $s?->type_capteur === $t ? 'selected':'' }}>{{ $t }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="col-md-4">
                        <label class="field-label">Interface de connexion</label>
                        <select class="form-select field-input" name="interface_connexion" disabled>
                            <option value="">Sélectionner...</option>
                            @foreach(['USB','Ethernet','WiFi','USB + WiFi'] as $i)
                            <option value="{{ $i }}" {{ $s?->interface_connexion === $i ? 'selected':'' }}>{{ $i }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="col-md-4">
                        <label class="field-label">Adresse IP (si réseau)</label>
                        <input type="text" class="form-control field-input" name="adresse_ip"
                               value="{{ $s?->adresse_ip }}" placeholder="192.168.x.x" disabled>
                    </div>
                    <div class="col-md-4 d-flex align-items-end gap-4 pb-2">
                        <div class="form-check">
                            <input class="form-check-input" type="checkbox" name="est_recto_verso" id="f_rv"
                                   value="1" {{ $s?->est_recto_verso ? 'checked':'' }} disabled>
                            <label class="form-check-label small" for="f_rv">Recto-Verso automatique</label>
                        </div>
                        <div class="form-check">
                            <input class="form-check-input" type="checkbox" name="a_chargeur_auto" id="f_adf"
                                   value="1" {{ $s?->a_chargeur_auto ? 'checked':'' }} disabled>
                            <label class="form-check-label small" for="f_adf">Chargeur auto (ADF)</label>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="modal-content border-0 shadow-lg" style="border-radius:14px">
            ...
            <form id="affectationForm">
                ...
                <div class="modal-body px-4 py-3">
                    ...
                    {{-- Summary cards identical to computers --}}
...
.aff-type-card { cursor:pointer; transition:all .2s; }
...
.nav-tabs .nav-link { border:none; border-bottom:2px solid transparent; color:#64748b; border-radius:0; transition:all .2s; }
...
$(function () {
    ...
    function quickAddMarque() {
        Swal.fire({
            ...
            input: 'text',
            inputPlaceholder: 'Libellé de la marque',
            showCancelButton: true,
            confirmButtonText: 'Ajouter',
            preConfirm: (libelle) => {
                if (!libelle) { Swal.showValidationMessage('Le libellé est requis'); return; }
                return $.post(route('parc-info.scanners.store-marque'), { _token: csrfToken, libelle });
            }
        }).then(res => {
            if (res.isConfirmed && res.value?.success) {
                const m = res.value.data;
                $('[name="marque_id"]').append(`<option value="${m.id}" selected>${m.libelle}</option>`).val(m.id);
            }
        });
    }

    $('#btn-add-marque-show').on('click', quickAddMarque);
    ...
    function setEditMode(on) {
        ...
        $('#btn-add-marque-show').toggleClass('d-none', !on);
    ...
    $('#ficheForm').on('submit', function (e) {
        ...
        $.ajax({
            url   : route('parc-info.scanners.update', equipementId),
            method: 'PUT',
            data  : $(this).serialize(),
            success: (res) => {
                if (res.success) {
    ...
    $('#affectationForm').on('submit', function (e) {
        ...
        $.post(route('parc-info.scanners.store-affectation'), data, (res) => {
            if (res.success) {
    ...
    $(document).on('click', '.dropdown-item[data-statut]', function(e) {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: route('parc-info.scanners.update-statut', equipementId),
                    method: 'PATCH',
    ...
    $('#btn-desaffecter').on('click', function() {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.post(route('parc-info.scanners.desaffecter', equipementId), { motif: result.value, _token: csrfToken }, (res) => {
