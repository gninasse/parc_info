...
@section('breadcrumb')
    ...
    <li class="breadcrumb-item"><a href="{{ route('parc-info.parefeux.index') }}">Pare-feux</a></li>
                        ...
                        <div class="card-body">
                            <div class="row g-3">
                                ...
                                <div class="col-md-6">
                                    ...
                                    <input type="text" class="form-control" name="adresse_ip" value="{{ $reseau->adresse_ip ?? '' }}" disabled id="f_adresse_ip" placeholder="Ex: 192.168.1.99">
                                </div>
                                <div class="col-md-6">
                                ...
                                <div class="col-md-3">
                                    ...
                                    <input type="number" class="form-control" name="nb_ports" value="{{ $reseau->nb_ports ?? '' }}" disabled id="f_nb_ports" placeholder="Ex: 8">
                                </div>
                                <div class="col-md-3">
                                ...
                                <div class="col-md-6">
                                    <label class="form-label small fw-bold text-muted">Version OS / Firmware</label>
                                    <input type="text" class="form-control" name="version_firmware" value="{{ $reseau->version_firmware ?? '' }}" disabled id="f_version_firmware" placeholder="Ex: FortiOS v7.2.4">
                                </div>
                                <div class="col-md-6 mt-auto">
                                    <div class="form-check">
                                        <input class="form-check-input" type="checkbox" name="est_poe" id="f_est_poe" value="1" {{ ($reseau->est_poe ?? false) ? 'checked' : '' }} disabled>
                                    ...
                                    <div class="form-check mt-2">
                                        ...
                                        <label class="form-check-label small fw-bold text-muted" for="f_est_manageable">Manageable (SSH/HTTPS/CLI)</label>
                                    </div>
                                </div>
                            </div>
                        </div>
        ...
        <div class="card border-0 shadow-sm" style="border-radius:12px">
            <div class="card-body p-4">
                ...
                <div class="text-center py-5">
                    ...
                    <p class="text-muted fw-semibold">Ce pare-feu n'est affecté à aucun local technique</p>
        ...
        <div class="modal-content border-0 shadow-lg" style="border-radius:14px">
            ...
            <form id="affectationForm">
                ...
                <div class="modal-body px-4 py-3">
                    ...
                    <p class="text-muted small mb-4">Associez ce pare-feu à un local ou un espace technique spécifique (salle serveur, baie, etc.).</p>
...
@push('js')
<script type="module" src="{{ asset('js/modules/parc-info/parefeux/index.js') }}?v={{ time() }}"></script>
...
$(function(){
    ...
    $('#btn-save-fiche').on('click', function(){
        ...
        $.ajax({
            url: route('parc-info.parefeux.update', '{{ $equipement->id }}'),
            method: 'PUT',
            data: $.param(formData),
            success: (res) => {
                if(res.success){
    ...
    $('[data-action="update-statut"]').on('click', function(e){
        ...
        Swal.fire({
            ...
            input: 'textarea',
            inputPlaceholder: 'Ex: Déploiement pare-feu, Maintenance technique...',
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: route('parc-info.parefeux.update-statut', '{{ $equipement->id }}'),
                    method: 'PATCH',
    ...
    $('#btn-desaffecter').on('click', function(){
        Swal.fire({
            title: 'Désaffecter le Pare-feu ?',
            text: 'Le pare-feu sera remis en stock. Indiquez le motif :',
            input: 'textarea',
            showCancelButton: true,
            ...
            if (result.isConfirmed) {
                $.post(route('parc-info.parefeux.desaffecter', '{{ $equipement->id }}'), {
    ...
    $('#affectationForm').on('submit', function(e){
        ...
        $.post(route('parc-info.parefeux.store-affectation'), $(this).serialize(), (res) => {
            if (res.success) {
