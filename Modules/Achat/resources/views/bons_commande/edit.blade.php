...
@section('title', 'Modifier Bon de Commande - Achat')
...
@section('breadcrumb')
    ...
    <li class="breadcrumb-item"><a href="{{ route('achat.bons-commande.show', $bonCommande->id) }}">{{ $bonCommande->numero_commande }}</a></li>
...
<div class="row g-3">
    <div class="col-lg-12">
        <form id="form-create-bc" autocomplete="off" novalidate>
            @csrf
            <input type="hidden" id="bc-id" name="id" value="{{ $bonCommande->id }}">
                    ...
                    <div class="row g-3">
                        <div class="col-md-6">
                            ...
                            <div class="input-group input-group-sm">
                                <input type="hidden" name="fournisseur_id" id="fournisseur_id" value="{{ $bonCommande->fournisseur_id }}" required>
                                <input type="text" class="form-control" id="fournisseur_name_display" placeholder="Sélectionner un fournisseur..." readonly value="{{ $bonCommande->fournisseur->nom }}" required>
                                <button class="btn btn-outline-primary" type="button" id="btn-choose-fournisseur">
                                    <i class="fas fa-search me-1"></i>Choisir
                                </button>
                                <button class="btn btn-outline-danger" type="button" id="btn-clear-fournisseur">
                                    <i class="fas fa-times"></i>
                                </button>
                            </div>
                            <div id="fournisseur_code_display" class="small text-muted mt-1">
                                Code Fournisseur : <span class="fw-semibold font-monospace" id="fournisseur_code_text">{{ $bonCommande->fournisseur->code }}</span>
                            </div>
                        ...
                        <div class="col-md-6">
                            ...
                            <input type="date" name="date_commande" class="form-control form-control-sm" value="{{ $bonCommande->date_commande->toDateString() }}" required>
                        ...
                        <div class="col-md-12">
                            ...
                            <textarea name="commentaire" class="form-control form-control-sm" rows="2" placeholder="Ajouter des notes, spécifications pour la livraison...">{{ $bonCommande->commentaire }}</textarea>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="d-flex justify-content-end gap-2 mt-4">
                <a href="{{ route('achat.bons-commande.show', $bonCommande->id) }}" class="btn btn-sm btn-outline-secondary rounded-1 px-4">Annuler</a>
