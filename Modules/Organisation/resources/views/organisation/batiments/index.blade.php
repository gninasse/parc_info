...
@section('header', 'Gestion des Bâtiments')
...
$(document).ready(function() {
    $('#btn-toggle-status').on('click', function() {
        const selections = $('#batiments-table').bootstrapTable('getSelections');
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: route('organisation.batiments.toggle-status', id),
                    type: 'PATCH',
                    headers: {
                    ...
                    success: function(response) {
                        if (response.success) {
                            ...
                            $('#batiments-table').bootstrapTable('refresh');
                        } else {
                            Swal.fire({
                                icon: 'error',
                                title: 'Erreur',
...
<div class="card">
    ...
    <div class="card-body">
        ...
        <table id="batiments-table"
               data-toggle="table"
               data-url="{{ route('organisation.batiments.data') }}"
            ...
            <thead>
                <tr>
                    ...
                    <th data-field="nombre_etages" data-sortable="true">Nb Étages</th>
...
@include('organisation::organisation.batiments._modal')
@stop
...
<script>
    window.batimentRoutes = {
        data: "{{ route('organisation.batiments.data') }}"
    };
...
<script type="module" src="{{ asset('js/modules/organisation/batiments/index.js') }}"></script>
@endpush
